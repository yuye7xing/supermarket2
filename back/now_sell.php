...
$sql=$e_sql." and m_commodity.mc_name LIKE '%".$commodity_name."%' and DATEDIFF (DATE_FORMAT(NOW(),'%Y-%m-%d'),m_time_sell.m_sell_end)<0 and DATEDIFF (DATE_FORMAT(NOW(),'%Y-%m-%d'),m_time_sell.m_time_sell)>=0" ;
$page_size=10;
...
while ($r=mysqli_fetch_row($result)){
	...
	$query2="SELECT count(*)  FROM `mc_detail` WHERE `mc_id` = $mc_id AND `state` = 1";
	$re2=mysqli_query($con, $query2);
	if($re3=mysqli_fetch_row($re2)){
		$num1=$re3[0];
	}
	...
	echo "<TD align='center'>".$num1."</TD>";
	echo "<TD align='center'>".$num0."</TD>";
	echo "<TD width='160' align='center'><input type=\"button\" style='height:21px; font-size:12px' onClick=\"now_gettddate()\" value=\"销售详情\"/> </TD></TR>";
}?>
